...
具体例も見ておこう。
たとえば列 $a = (1,2,3,2,9,3,4,5,6,7)$ かつ $k = 3$ のときの結果の列 $b$ を図にすると次のようになる。
...
列 $a = (a_0, a_1, \dots, a _ {n-1})$ の全体での最小値 $a_i = \min_j a_j$ を含むような区間から作られる列のみ数え、その後その最小値を削除してできる左右の列について再帰することにする。
