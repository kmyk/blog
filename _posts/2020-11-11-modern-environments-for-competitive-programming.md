...
競プロをするための環境、つまり「それがないと競プロをすることができないようなもの」ではありません。
...
行頭の `$` は「この行はターミナルに入力すべきコマンド文字列である」ということを示す記号 (たとえば円記号 `￥` のようなもの) であり、実際には入力する必要はありません。
...
さらに `-D_GLIBCXX_DEBUG` `-fsanitiz=undefined` などを指定すると、実行時に (実行速度と引き換えに) 未定義処理などを検出して警告してくれます。
